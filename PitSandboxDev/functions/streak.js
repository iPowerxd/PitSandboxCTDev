...
register('command', (arg1) => {
    ...
    switch (arg1) {
        case 'show':
            ...
            info.push(`&9Mastery XP: ${formatNumber(streaksession.mastery)}`)
            info.push('\n')
            ...
            if (streaksession.totaleggs > 0) {
                ...
                info.push(`§bElectrified Hits: ${streaksession.onetap}/${streaksession.totaleggs} §7(${Math.round(streaksession.onetap / streaksession.totaleggs * 100)}%)`)
...
}).setTabCompletions(['show', 'clear']).setName('session')
...
const recapStreak = () => {
    ...
    } if (currentstreak.mastery != 0) {
        streakinfo.push(`${currentstreak.mastery != 0 ? 'Mastery XP: &9' : ''}${currentstreak.mastery != 0 ? currentstreak.mastery : ''}`)
    }
    ...
    if (scoreboard.find(l => l.startsWith("Status: ") && !l.startsWith("Status: Fighting") && !l.startsWith("Status: Idling") && !l.startsWith("Status: Bountied") && !l.startsWith("Status: Strength"))) {
        ...
        if (megastreak == 'Made of Time') megastreak = '&5' + megastreak
        if (megastreak == 'Gilded') megastreak = '&6' + megastreak
...
register("chat", (player, xp, gold, mastery, event) => {
    if (!onSandbox()) return;
    if (!Settings.toggleSandboxHUD) return;
    cancel(event);
    strength()
    ...
    if (parseFloat(gold) != NaN && parseFloat(gold)) {
        ...
        goldrequire += parseFloat(gold)
        streaksession.coins += parseFloat(gold)
    } if (parseFloat(mastery) != NaN && parseFloat(mastery)) {
        currentstreak.mastery += Number(mastery)
        streaksession.mastery += Number(mastery)
    }
}).setChatCriteria("KILL! on ${player} +${xp} +$${gold} +${mastery} Mastery XP");

register("chat", (mult, player, xp, gold, mastery, event) => {
    if (!onSandbox()) return
    if (!Settings.toggleSandboxHUD) return;
    if (mult.split(" ").length > 1) return;
    cancel(event)
...
}).setChatCriteria("${mult} KILL! on ${player} +${xp}XP +$${gold} +${mastery} Mastery XP")
...
}).setChatCriteria("MULTI KILL! (${mult}) on ${player} +${xp}XP +$${gold} +${mastery} Mastery XP")

//Stupid Massacre stuff
...
////MULTI KILL! (260) on HowMyLove +710.66XP +$416.85 +1 Mastery XP +1.6 Streak
//MULTI KILL! (121) on 16Fov +325XP +$174 +1 Mastery XP +1.6 Streak
//Plethora and Others
...
register('command', () => {
    ChatLib.chat(currentstreak.mastery)
}).setName('a')
    ...
    register('step', () => {
        ...
        if (!streaking || !inMid(Player.asPlayerMP())) streakinglines = [`${Settings.hudGroupColor}&nStreaking Info`]
            ...
            if (startstreaktime) {
                ...
                streakinfo.push(`${Settings.hudTextColor}Mastery XP:&9 ${currentstreak.mastery}`)
            } if (startstreaktime && streakkills) {
                let kps = Math.floor(streakkills / ((Date.now() - startstreaktime) / 1000) * 10) / 10;
                let kpm = formatNumber(Math.floor(streakkills / ((Date.now() - startstreaktime) / 1000 / 60)));
                let kph = formatNumber(Math.floor(streakkills / ((Date.now() - startstreaktime) / 1000 / 60 / 60)));
                ...
                if (megastreak == "Nightmare") megastreak = "&1" + megastreak;
                if (megastreak == "Hermit") megastreak = "&9" + megastreak;
                if (megastreak == 'Made of Time') megastreak = '&5&l' + megastreak
                if (megastreak == 'Gilded') megastreak = '&6&l' + megastreak
