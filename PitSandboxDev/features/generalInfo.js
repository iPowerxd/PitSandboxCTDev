...
const contractActive = () => {
    ...
    const tab = TabList.getHeader().split('\n')
    for (let i = 0; i < tab.length; i++) {
        if (tab[i].includes('CONTRACT!')) return tab[i].split('CONTRACT! ')[1]
    } return false
}

function contractColors(contract) {
    if (contract.toLowerCase().includes('bot')) {
        return `&c${contract.replace('/', '&7/&c')}`
    } else if (contract.toLowerCase().includes('mega')) {
        return `&c${contract.replace('/', '&7/&c')}`
    } else if (contract.toLowerCase().includes('uber')) {
        return `&d${contract.replace('/', '&7/&d')}`
    } else if (contract.toLowerCase().includes('mine')) {
        return `&7${contract.replace('/', '&f/&7')}`
    } else if (contract.toLowerCase().includes('night')) {
        return `&9${contract.replace('/', '&7/&9')}`
    } else {
        return `&f${contract}`
    }
}
...
function alternateWinLose() {
    return win ? "&2&kk&r &aWin &2&kk&r" : "&4&kk&r &cLose &4&kk&r"
}

function majorEventColors(event) {
    if (event.toLowerCase().includes('rewards')) {
        return event.replace('2x Rewards', '&62&bx &2Rewards')
    } else if (event.toLowerCase().includes('blood')) {
        return event.replace('Blood Bath', '&4Blood &cB&4a&ct&4h')
    } else if (event.toLowerCase().includes('auction')) {
        return (event.replace('Auction', '&6Auction $?,???'))
    } else if (event.toLowerCase().includes('team')) {
        return (event.replace('Team Destroy', '&cT&2e&aa&bm &cDestroy'))
    } else if (event.toLowerCase().includes('gamble')) {
        return (event.replace('Gamble', `${color}Gamble ${alternateWinLose()}`))
    }
}
//blood bath, auction, 2x rewards, gamble, and team destroy
//Next Major: (time) Gamble $ Win $ || $ Lose $
/* register('step', () => {
    ChatLib.chat(alternateWinLose())
}).setFps(4) */
...
register("step", () => {
    setTimeout(() => {
        ...
        if (scoreboard.find(l => l.startsWith("Prestige: ")) && scoreboard.find(l => l.startsWith("Level: "))) {
            ...
            general.push(`${Settings.hudTextColor}Yummy Cookie: ${yummyTime().toUpperCase()}`)
        } if (contractActive()) {
            general.push(`${Settings.hudTextColor}Contract: ${contractColors(ChatLib.removeFormatting(contractActive()))}`)
            ...
            if (Settings.toggleSimpleHUD) {
                if (nextmajor > Date.now() && majorname) general.push(`${Settings.hudTextColor}Next Major: &e${msToTime(nextmajor - Date.now())}&7,&r ${majorEventColors(majorname)}`)
            } else {
                if (nextmajor > Date.now()) {
                    ...
                    general.push(Settings.hudTextColor + "Major Name: &e" + majorEventColors(majorname));
                }
            }
        }
